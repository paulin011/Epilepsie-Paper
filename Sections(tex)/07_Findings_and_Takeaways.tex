...
This section synthesizes evidence from 13 primary studies on wearable seizure detection and forecasting, identifying patterns that inform clinical practice and future research.

\subsection{Executive Summary}

\begin{itemize}
    \item \textbf{Convulsive seizure detection achieves clinical viability:} Non-EEG biosignals, primarily accelerometry and cardiac modalities, detect convulsive seizures with 71.6--100\% sensitivity. Only two of nine detection studies meet the clinical false alarm rate standard of $<0.5$/h, while ECG-only methods suffer extremely high false alarm rates (1.91--39.75/h).

    \item \textbf{Seizure forecasting remains immature:} Only 43.5--83\% of patients achieve better-than-chance performance, indicating a fundamental responder problem. Most studies remain at ILAE Phase 1--2 with limited evidence supporting real-world deployment.
    ...
    \item \textbf{False alarms constitute the primary barrier:} Nighttime false alarm rates (1/61 nights) consistently outperform daytime (1/9 days), yet no multimodal study has achieved $<0.5$/h across both sleep and wake states despite evidence that heart rate increases approximately 100 seconds before movement onset.
\end{itemize}

\subsection{Detection Performance}

Clinically viable seizure detection remains challenging, with only two approaches achieving false alarm rates (FAR) within the actionable range ($<$0.5/h). Fine (2025) reported the first automated tonic seizure detector, achieving 100\% sensitivity at 0.023/h FAR with 14-second mean latency. Dong (2026) demonstrated that nocturnal monitoring reduces false positives substantially, attaining 71.6\% sensitivity at 0.165/h using CNN-LSTM with attention mechanisms. By contrast, Spahr (2025) achieved 96\% sensitivity for convulsive seizures but required a 30-model ensemble CNN to maintain $<$1/8 nights FAR.

ECG-based detection, despite achieving high sensitivity (98.16\%), consistently fails clinical utility thresholds due to motion artifacts inflating FAR to 1.91--39.75/h. This represents 20--200$\times$ above acceptable limits and renders ECG-only approaches impractical for ambulatory use. The modality effectiveness hierarchy clearly favors accelerometer-based approaches, used in 8 of 13 studies for convulsive seizure detection, while multimodal combinations (ACC + cardiac) show promise for improving specificity.
...
Despite algorithmic sophistication, validation rigor has not kept pace. Only three of 13 studies employed prospective designs, and LOSO cross-validation remains rare. Eleven of 13 studies conducted validation in epilepsy monitoring units under controlled conditions that minimize motion artifacts and environmental noise. The two studies testing systems in home environments both reported performance degradation compared to EMU results.
...
Emerging technical directions include attention mechanisms for dynamic channel weighting, matrix profile approaches for unsupervised anomaly detection, and ensemble methods with tunable sensitivity thresholds. However, real-world deployment remains elusive: most algorithms run offline on servers rather than embedded devices. Edge optimization has received minimal attention despite being critical for clinical translation, with only one study demonstrating real-time inference on smartwatch hardware.

\subsection{Clinical Takeaways}

Table~\ref{tab:detection-performance} summarizes the best-performing detection systems. Only two approaches achieve clinically acceptable false alarm rates.
