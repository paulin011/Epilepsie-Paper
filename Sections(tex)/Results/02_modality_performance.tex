...
Multi-modal movement detection results in excellent sensitivity in controlled settings. \textcite{fineDetectionKeyAutomated2025} used a 6-axis band combining ACC and gyroscope to achieve 100\% sensitivity with 0.023/h FPR in an independent test set. However, the test set contained only 10 seizures from 3 patients.
...
Multi-modal approaches combining movement and autonomic signals show mixed results. \textcite{dongTwoLayerEnsembleMethod2022} combined ACC and PPG in a two-stage CNN-LSTM architecture, achieving 71.6\% sensitivity with 0.165/h FPR. \textcite{wangEpilepticSeizureDetection2025} combined ACC, gyroscope, SEMG, and EDA to achieve 56.4\% to 95.3\% sensitivity depending on the feature set.

\textcite{wangEpilepticSeizureDetection2025} directly compared modality performance. Attitude angle features (PITCH and ROLL) outperformed raw accelerometer and gyroscope data for seizure detection. This finding suggests that feature engineering can enhance the value of standard movement sensors.
